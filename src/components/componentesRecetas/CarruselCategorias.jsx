...
const categorias = [
  { nombre: "Panes", imagen: "/assets/Panes.jpg" },
  { nombre: "Pastas", imagen: "/assets/PASTAS.jpg" },
  { nombre: "Tortas y dulces", imagen: "/assets/dulces.jpg" },
  { nombre: "Masas", imagen: "/assets/Masas.jpg" },
];

export default function CarruselCategorias({ setCategoria }) {
  const [index, setIndex] = useState(0);
  const [paused, setPaused] = useState(false);

  const prevSlide = () => {
    setIndex((prev) => (prev === 0 ? categorias.length - 2 : prev - 1));
  };

  const nextSlide = () => {
    setIndex((prev) => (prev >= categorias.length - 2 ? 0 : prev + 1));
  };
  ...
  const visible = categorias.slice(index, index + 2);
  if (visible.length < 2) visible.push(categorias[0]);
>>>>>>> yamila-dev
...
=======
      ...
      <div className={styles.carouselInner}>
        {visible.map((cat, i) => (
          ...
          >
            <img src={cat.imagen} alt={cat.nombre} className={styles.image} />
          </div>
        ))}
      </div>
